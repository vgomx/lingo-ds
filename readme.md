...
- **Fonts.** No binaries were supplied, so `Baloo 2` stands in for the wordmark's display face; `Nunito Sans` is the UI face; `JetBrains Mono` carries phonetics and data. All three are self-hosted WOFF2 in `assets/fonts/`, generated by `npm run build:fonts` — see "Ask" at the bottom for what would replace them.
...
- **Streaks.** The streak is the one number that measures the habit rather than the collection, and it is global: one exercise anywhere in the app — a card graded, a form answered — keeps the day. That makes it worth saying out loud. Name what someone has done, mark a genuine milestone, and make the next day easy to start: a short way in, a clear "one answer is enough", a suggestion of the quickest thing available.
  ...
  **Missing a day is allowed for.** Nobody practises every day, and a counter that punishes an ordinary week is measuring the wrong thing. Forgiveness can be built into the definition — a rest day the streak simply survives, needing nothing claimed — or offered as a day held open by something bought in advance. Both are fine, and the second is better when the reader chooses it rather than being sold it.

  **Repair is held, not applied.** An extension is bought ahead of time and spent on its own the day it is needed, because somebody who forgot to practise is by definition not there to press anything — a repair that has to be noticed and claimed asks for the attention whose absence caused the problem. So it waits in hand, a small inventory with a low ceiling: enough to cover an ordinary lapse, never enough to bank a fortnight of absence, and spending one frees the slot so the currency always has somewhere to go.

  **It is bought with practice.** Where there is a price it is paid in something already done: points earned by answering, never money, and never anything that can be topped up. That keeps the exchange inside the habit — the only way to cover a day is to have practised on another one — and it is why a price is not the same as pressure. Two rules follow. Spending must achieve something: if the gap is wider than what is held, nothing is spent rather than an inventory burned on a run that ended anyway. And a covered day is marked as covered wherever the history is drawn, in its own mark rather than a shade of a practised one — it holds the streak, and the record still says what happened.
  ...
  Two consequences worth writing down. The app has no server, no accounts and no payments, so a streak can never be measured against other people and nothing about it can be sold — that is architecture, not virtue, and it should not be mistaken for a rule doing work. And a lapse gets no commentary: the number resets without a post-mortem, and `0 days` is never shown at all — an empty streak is a fact nobody needs, so a lapsed run prints nothing rather than a zero.
...
| `assets/fonts/` | Self-hosted WOFF2 + the OFL text for each family (generated) |
...
**Send real font binaries.** Baloo 2 / Nunito Sans / JetBrains Mono are stand-ins chosen from the logo's letterforms. If the wordmark was set in a licensed face, drop the `.woff2` files into `assets/fonts/` and point `tokens/fonts.css` at them — the substitution is the open question, not the plumbing.

**The plumbing is done.** `tokens/fonts.css` is generated by `npm run build:fonts` and holds real `@font-face` rules against local WOFF2 files, so a consuming app can precache its own type and renders in the right face offline and on a first paint. Google Fonts is no longer contacted at runtime, which also stops every reader's browser announcing itself to a third party to load a page. The licences ship in `assets/fonts/` beside the binaries, as the OFL requires of anyone redistributing them.
